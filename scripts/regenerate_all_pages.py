...
function askSug(q){{document.getElementById('sugs').innerHTML='';addMsg('user',q);showT();setTimeout(function(){{removeT();addMsg('butler','ご質問ありがとうございます。\\n\\n※ サクラAIはAPI接続後に実動作します。');renderSugs();}},1200);}}
function sendMsg(){{var i=document.getElementById('chat-inp'),q=i.value.trim();if(!q)return;i.value='';document.getElementById('sugs').innerHTML='';addMsg('user',q);showT();setTimeout(function(){{removeT();addMsg('butler','ご質問ありがとうございます。\\n\\n※ サクラAIはAPI接続後に実動作します。');renderSugs();}},1500);}}
